...
### E-Commerce & Boutiques Partenaires
...
| Technologie | Rôle |
|-------------|------|
| React Native + Expo SDK ~50 | Framework cross-platform Android & iOS |
| React Navigation v6 | Navigation (stacks, tabs, modals) |
| React Native Maps | Cartes OpenStreetMap — aucune clé API requise |
...
| Expo Secure Store | Clés d'authentification chiffrées |
...
| Technologie | Rôle |
|-------------|------|
| React 18 + Vite | Interface rapide et moderne |
...
- Protection anti-brute-force (blocage par IP + identifiant)
...
- `merchant` — commerçant e-commerce
...
- Paiement en un tap avec **−5 % de réduction**
...
- **Paramètres** : taux de commission, frais de réservation, règles cashback
...
│   │   └── security.middleware.js  Brute-force, rate-limit
...
│   ├── pm2.config.js            Config déploiement PM2
...
│       │   ├── Market/          EcommerceScreen, CarMarket
...
│       ├── Settings.jsx         Paramètres plateforme
│       └── ...                  15 pages au total
│
...
```
API backend   →  PM2   →  port 5000
Panel admin   →  PM2 serve (--spa)  →  port 3001
```
