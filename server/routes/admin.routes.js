    ...
    try {
        ...
        if (status === 'approved' && !v.rental_car_id) {
            const car = await RentalCar.create({
                ...
                year:           v.year,
                color:          v.color,
                license_plate:  v.plate_number || 'À-DÉFINIR',
                seats:          v.seats || 5,
                ...
                pickup_lat:     0,
                pickup_lng:     0,
                pickup_address: 'À configurer',
                available_from: new Date(),
                available_until: new Date(Date.now() + 365 * 86400000),
                status:         'pending_approval',
                photos:         v.photos ? Object.values(v.photos).filter(Boolean) : [],
