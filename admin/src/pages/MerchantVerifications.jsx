...
const TYPE_META = {
    partner:    { label: 'Partenaire',       color: '#00897B', bg: '#D4F5F2', icon: '🏪' },
    car_seller: { label: 'Vendeur auto',     color: '#7B1FA2', bg: '#F3E5F5', icon: '🚗' },
};
...
export default function MerchantVerifications() {
    ...
    const load = useCallback(async () => {
        ...
        try {
            ...
            if (typeFilter)   params.merchant_type = typeFilter;
    ...
    return (
        <div className="page">
            <div className="page-header">
                <div>
                    <h1 className="page-title">Vérifications Marchands</h1>
                    <p className="page-subtitle">KYC · Partenaires acceptant les paiements Ombia & vendeurs automobiles</p>
                </div>
            </div>

            {/* Type filter tabs */}
            <div style={{ display: 'flex', gap: 8, marginBottom: 14 }}>
                {[{ key: '', label: 'Tous' }, { key: 'partner', label: '🏪 Partenaires' }, { key: 'car_seller', label: '🚗 Vendeurs auto' }].map(t => (
                    <button
                        key={t.key}
            ...
            <div style={{ display: 'flex', gap: 16 }}>
                        ...
                        <table className="data-table">
                            ...
                            <tbody>
                                ...
                                {verifications.map(v => {
                                    ...
                                    const tm  = TYPE_META[v.merchant_type]  || TYPE_META.partner;
                                    ...
                                    return (
                                        <tr key={v.id} style={{ cursor: 'pointer', background: selected?.id === v.id ? '#F0F7FF' : undefined }} onClick={() => openDetail(v)}>
                                            ...
                                            <td>
                                                <span style={{ background: tm.bg, color: tm.color, padding: '3px 10px', borderRadius: 8, fontWeight: 700, fontSize: 12 }}>
                                                    {tm.icon} {tm.label}
                                                </span>
                                            </td>
                                            <td>
                                                <div style={{ fontWeight: 600 }}>{v.business_name || '—'}</div>
                                                <div style={{ fontSize: 11, color: '#888' }}>{v.business_type}</div>
                                            </td>
                ...
                {/* Detail panel */}
                {selected && (() => {
                    const tm = TYPE_META[selected.merchant_type] || TYPE_META.partner;
                    return (
                        <div style={{ width: 380, flexShrink: 0, background: '#fff', borderRadius: 14, border: '1.5px solid #E8EAF0', padding: '18px 20px', maxHeight: '80vh', overflowY: 'auto' }}>
                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 14 }}>
                                <div>
                                    <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                                        <span style={{ background: tm.bg, color: tm.color, fontSize: 11, fontWeight: 700, padding: '2px 8px', borderRadius: 6 }}>{tm.icon} {tm.label}</span>
                                    </div>
                            ...
                            <div style={{ background: '#FAFAFA', borderRadius: 10, padding: '12px 14px', marginBottom: 14 }}>
                                <div style={{ fontSize: 11, fontWeight: 700, color: '#888', textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 8 }}>Activité</div>
                            ...
                            {selected.bank_info && Object.values(selected.bank_info).some(Boolean) && (
                                <div style={{ background: '#F3FFFD', borderRadius: 10, padding: '12px 14px', marginBottom: 14, border: '1px solid #B2DFDB' }}>
                                    <div style={{ fontSize: 11, fontWeight: 700, color: '#00897B', textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 8 }}>Compte bancaire</div>
                                ...
                                <div style={{ marginBottom: 14 }}>
                                    ...
                                    {selected.status === 'approved' && (
                                        <div style={{ marginTop: 8, fontSize: 12, color: '#2E7D32', background: '#E8F5E9', padding: '6px 10px', borderRadius: 8 }}>
                                            ✓ Compte marchand activé — service débloqué pour l'utilisateur
                                        </div>
                                    )}
                                </div>
                            )}
                            ...
                            <div style={{ marginBottom: 10 }}>
                                <div style={{ fontSize: 11, fontWeight: 600, color: '#555', marginBottom: 4 }}>Note pour le marchand</div>
